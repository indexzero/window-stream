...
TimeWindow.prototype.write = function (data) {
  ...
  //
  // Increment the number of uncounted events
  // in this interval.
  //
  this.uncounted++;  
  this.data.push(data);
  ...
  if (this.fixed) {
    for (remove = 0; remove < this.data.length; remove++) {
      if (!this.data[remove].time) {
        continue;
      }
      else if ((now - new Date(this.data[remove].time)) > this.duration) {
        break;
      }
    }
